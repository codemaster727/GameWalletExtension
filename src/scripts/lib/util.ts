...
function checkForError() {
  const { lastError }: { lastError?: any } = browser.runtime;
  if (!lastError) {
    return undefined;
  }
...
function getChainType(chainId: any) {
 ...
 * @returns
 */
function checkAlarmExists(alarmList: any, alarmName: string) {
  return alarmList.some((alarm: any) => alarm.name === alarmName);
}
...
export const isValidDate = (d: any) => {
  return d instanceof Date && !isNaN(Number(d.toString()));
};
