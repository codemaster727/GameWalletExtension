...
// import { updateBackgroundConnection } from '../App';
import { _setBackgroundConnection } from 'src/store/action-queue';
import {
...
console.log('Just before start UI');
...
async function start() {
  ...
  function initializeUiWithTab(tab: any) {
    initializeUi(tab, connectionStream, (err: any, store: any) => {
      if (err) {
...
function initializeUi(activeTab: any, connectionStream: any, cb: any) {
  connectToAccountManager(connectionStream, (err: any, backgroundConnection: any) => {
    if (err) {
      cb(err, null);
      return;
...
async function displayCriticalError(errorKey: any, err: any, metamaskState: any) {
  ...
  (button as HTMLElement).addEventListener('click', (_) => {
...
function connectToAccountManager(connectionStream: any, cb: any) {
  console.log('connectionStream1:', connectionStream);
  const mx = setupMultiplex(connectionStream);
  // const controllerConnectionStream = mx.createStream('controller');
  // console.log('controllerConnectionStream:', controllerConnectionStream);
 ...
 */
function setupControllerConnection(controllerConnectionStream: any, cb: any) {
