...
export default class ExtensionPlatform {
  ...
  async openTab(options: any) {
    const newTab = await browser.tabs.create(options);
    return newTab;
  }

  async openWindow(options: any) {
    const newWindow = await browser.windows.create(options);
    return newWindow;
  }

  async focusWindow(windowId: any) {
    await browser.windows.update(windowId, { focused: true });
  }

  async updateWindowPosition(windowId: any, left: any, top: any) {
  ...
  async closeCurrentWindow() {
    ...
    browser.windows.remove(windowDetails.id as number);
  }
  ...
  _showFailedTransaction(txMeta: any, errorMessage: string) {
    const nonce = parseInt(txMeta.txParams.nonce, 16);
